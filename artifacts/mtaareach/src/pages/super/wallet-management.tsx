  ...
  return (
    <div className="space-y-6">
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="h-20 text-center text-muted-foreground">No wallets found.</TableCell>
                  </TableRow>
                ) : (
                  wallets?.map((w) => (
                    <TableRow key={w.id}>
                      <TableCell>
                        <p className="font-medium text-sm">{w.tenantName ?? `Tenant #${w.tenantId}`}</p>
                      </TableCell>
                      <TableCell className={`text-right font-mono font-bold ${w.balance < 1000 ? "text-amber-600" : ""}`}>
                      ...
                      <TableCell className="text-right">
                        <Button size="sm" variant="outline" className="gap-1"
                          onClick={() => openTopUp(w.tenantId, w.tenantName ?? `Tenant #${w.tenantId}`)}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Top Up Wallet — {selectedName}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="e.g. Monthly allocation"
            ...
            <Button
              ...
              onClick={() => topUpMutation.mutate({
                data: {
                  ...
                  description: form.description || "Manual top-up"
                }
              })}
            >

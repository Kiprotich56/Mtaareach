...
function roleBadge(role: string) {
  ...
  return <Badge variant="outline" className={`text-xs ${map[role] ?? ""}`}>{label}</Badge>;
}
...
export default function Users() {
  ...
  const { data: usersPage, isLoading } = useListUsers({ page: 1, limit: 50 });
  ...
  const [form, setForm] = useState({ firstName: "", lastName: "", email: "", password: "", role: "field_agent" });
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>{isLoading ? "Loading…" : `${usersPage?.total ?? 0} users`}</CardDescription>
        </CardHeader>
        <CardContent>
